...
def test_finite_number_rejects_overflowing_int():
    assert ps_query._finite_number(10**400) is False
    assert ps_query._finite_number(1.5) is True
    assert ps_query._finite_number(True) is False
    assert ps_query._finite_number("x") is False
    assert ps_query._finite_number(float("inf")) is False
