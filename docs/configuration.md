...
| `uplink_kbps` | unthrottled | Head-wide upload budget (KiB/s) for every transfer leg that leaves this head — code snapshots, artifact publication, pulls; a site's `bwlimit_kbps` or a command's `--bwlimit` wins where present, and the head's own `local: true` node (a disk-to-disk copy) is never budgeted |
